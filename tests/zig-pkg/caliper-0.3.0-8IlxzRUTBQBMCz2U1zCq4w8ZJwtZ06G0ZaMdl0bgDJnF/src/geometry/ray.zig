...
pub fn ray_aabb_with_enter_exit(aabb: anytype, origin: @Vector(3, @TypeOf(aabb).child), invDir: InvDirection(@TypeOf(aabb).child)) struct { min: std.meta.Float(@bitSizeOf(@TypeOf(aabb).child)), max: std.meta.Float(@bitSizeOf(@TypeOf(aabb).child)) } {
    comptime {
        std.debug.assert(@TypeOf(aabb).primative_type == .AABB); // ensure aabb type
    }
    const flt_min = @as(@Vector(3, @TypeOf(aabb).child), @splat(-std.math.floatMax(@TypeOf(aabb).child)));
