...
test "transformed" {
    ...
    const p2 = Plane(f32).from_point_and_normal(vec.extract(transform.mul(vec.to_mat(@Vector(4, f32){ point[0], point[1], point[2], 1.0 })).column(0), 3), transform.extract(3, 3).mul(vec.to_mat(normal)).column(0));
